            ...
            <nav>
                ...
                <div className={`${isActive ? '-translate-x-0' : 'translate-x-[700px]'} fixed top-0 right-0 flex flex-col w-3/4 h-screen bg-black z-40 transition duration-500 ease-in-out p-10 md:hidden`}>
                    <div className="flex justify-between w-full px-5">
                        <h2 className='font-semibold text-lg font-montserrat self-start'>Menus</h2>
                ...
                <div className={`${isActive ? 'block' : 'hidden'} backdrop-blur-sm top-0 left-0 z-20 fixed w-full h-screen bg-transparent transition duration-500 ease-in-out`} onClick={() => setIsActive(prev => !prev)}></div>
            </nav>
        </div>
    </header>
    </>
